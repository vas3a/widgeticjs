(function(define){(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);throw new Error("Cannot find module '"+o+"'")}var f=n[o]={exports:{}};t[o][0].call(f.exports,function(e){var n=t[o][1][e];return s(n?n:e)},f,f.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
// UMD header
...
var Composition, api, auth, comps, config, guid, messageType, method, methods, queue, relayedEvents, _fn,
...
relayedEvents = ['click', 'dblclick', 'mousedown', 'mousemove', 'mouseup', 'mouseenter', 'mouseleave', 'mouseover', 'mouseout', 'wheel', 'keydown', 'keyup', 'keypress'];
...
Composition = function(holder, opt1, opt2) {
  ...
  if (options.edit_mode) {
    query.push('edit_mode');
  }
  if (options.resize) {
    query.push('resize=' + options.resize);
  }
  if (options.autoscale) {
    query.push('autoscale=' + options.autoscale);
  }
  if (query.length) {
  ...
  if (this._iframe.parentElement.parentElement.className.indexOf('wdgtc-wrap') === 0) {
    this._responsiveWrapper = this._iframe.parentElement.parentElement;
  }
  this._autoscale = options.autoscale !== 'off';
  return this;
};

Composition.prototype.updateSize = function(width, height) {
  if (!this._autoscale) {
    return;
  }
  if (this._responsiveWrapper && height) {
    this._responsiveWrapper.style.minHeight = height + 'px';
  }
  if (height) {
    this._iframe.style.height = height + 'px';
  }
  if (width) {
    return this._iframe.setAttribute('width', width);
  }
};
...
Composition.prototype.on = function(ev, callback) {
  ...
  if (__indexOf.call(relayedEvents, ev) >= 0) {
    this._sendMessage({
      t: 're',
      d: ev
    });
...
Composition.connect = function(id) {
  var _ref;
  return (_ref = comps[id.d]) != null ? _ref._ready() : void 0;
};
...
Composition.updateSize = function(data) {
  var _ref;
  return (_ref = comps[data.id]) != null ? _ref.updateSize(data.d.width, data.d.height) : void 0;
};
...
Composition.getComp = function(id) {
  return comps[id];
};
...
},{}],"UI/editor/index":[function(require,module,exports){
module.exports=require('sk8aR+');
},{}],"sk8aR+":[function(require,module,exports){
var Editor, api, config, editors, guid, pubsub, queue, relayedEvents,
  __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
  __slice = [].slice;
...
Editor.prototype.addContent = function(content) {
  this._sendMessage({
    t: 'cont',
    d: content
  });
...
},{"../../api":"pK1+ma","./../../../../bower/pubsub.js/src/pubsub.js":2,"./../../../../bower/queue-async/queue.js":3,"config":"ZaiTg0","utils/guid":"EadS8b"}],"UI/index":[function(require,module,exports){
...
},{"./composition":"aP+Ks/","./editor":"sk8aR+","./parse":"RadvXf","./plugin":"vqfNgY","./popup":"7FyBBT"}],"RadvXf":[function(require,module,exports){
...
resizeHolderTemplate = function(id, inline, styles, forIframeEmbed) {
  var inlineStyle;
  inlineStyle = inline ? 'display:inline-block;vertical-align:middle;' : '';
  if (forIframeEmbed) {
    return ("<div style=\"width:100%;" + inlineStyle + (styles.wrapStyle || '') + "\">") + ("<div style=\"position:relative; padding: 0;" + (styles.holdStyle || '') + "\">") + "$$$" + "</div>" + "</div>";
  } else {
    return "<div class=\"wdgtc-wrap\" data-wdgtc-id=\"" + id + "\" style=\"width:100%;" + inlineStyle + (styles.wrapStyle || '') + "\">			<div class=\"wdgtc-holder\" style=\"position:relative; padding: 0;" + (styles.holdStyle || '') + "\">			</div>		</div>";
  }
};
...
parse.wrapperHtml = function(options, forIframeEmbed) {
  var styles;
  if (forIframeEmbed == null) {
    forIframeEmbed = false;
  }
  styles = stylesFactory[options.resize](options.width, options.height);
  return resizeHolderTemplate(options.composition, options.inline, styles, forIframeEmbed);
};

parse.iframeStyle = 'position:absolute;top:0;left:0;width:100%;height:100%;';

embed = function(el) {
  var composition, holder, options,
    _this = this;
  options = {
...
},{"../utils/ready":"IiXnYl"}],"UI/parse":[function(require,module,exports){
module.exports=require('RadvXf');
},{}],"vqfNgY":[function(require,module,exports){
...
Plugin = (function() {
  ...
  Plugin.prototype.addEditorContent = function(d) {
    this._sendMessage({
      t: 'econt',
      d: d
    });
...
},{}],"UI/popup/index":[function(require,module,exports){
module.exports=require('7FyBBT');
},{}],"7FyBBT":[function(require,module,exports){
var Popup, ackMessage, aye, clientRectToObject, config, debounce, defs, event, extend, getCssValue, getOffset, getTextFromStyleElement, guid, json, loadSheet, loadSheets, newMessage, replyMessage, send, ucfirst, _getInfo,
...
loadSheets = function(el, sheets, optional) {
  var allSheets, def, loadedSheets, onLoad, sheet, total, _i, _len,
    _this = this;
  if (sheets == null) {
    sheets = [];
  }
  ...
  loadedSheets = 0;
  onLoad = function(ev) {
    if (ev.type === 'load' || optional.indexOf(ev.target.href) > -1) {
      ++loadedSheets;
    } else {
      def.reject(new Error("Popup could not be created because \"" + ev.target.href + "\" did not load!"));
    }
    if (loadedSheets === total) {
      return def.resolve();
    }
  };
  for (_i = 0, _len = allSheets.length; _i < _len; _i++) {
    sheet = allSheets[_i];
    loadSheet(sheet, el, onLoad);
  }
  setTimeout(def.reject.bind(null, new Error('Popup could not be created because CSS did not load')), 10000);
  return def.promise;
};
...
clientRectToObject = function(clientRect) {
  var key, res, value;
  res = {};
  if (!clientRect) {
    return res;
  }
  for (key in clientRect) {
    value = clientRect[key];
    res[key] = value;
  }
  return res;
};

_getInfo = function(popupIframe) {
  var _ref;
  return {
    popup: clientRectToObject(popupIframe.getBoundingClientRect()),
    widget: clientRectToObject((_ref = popupIframe._parentFrame) != null ? _ref.getBoundingClientRect() : void 0)
  };
};
...
Popup = (function() {
  ...
  Popup.onCreate = function(message, ev) {
    ...
    iframe.doPosition = function() {
      var msg;
      _this.doPosition(iframe, null);
      msg = {
        t: 'p',
        d: {
          event: 'reposition',
          name: name,
          response: {
            info: _getInfo(iframe)
          }
        }
      };
      return send(msg, iframe._parent);
    };
  ...
  Popup.onCreateDone = function(message, event) {
    var popupFrame, source;
    source = event.source;
    if (!(popupFrame = source.frames[message.d.name])) {
      while (source !== event.source.top) {
    ...
    return ackMessage(message, popupFrame);
  };

  Popup.onReposition = function(message, event) {
    this.popups[message.d.name].info = message.d.response.info;
    return this.popups[message.d.name].trigger('reposition');
  };
  ...
  function Popup(options) {
    ...
    this._saveInfo = __bind(this._saveInfo, this);
  ...
  Popup.prototype.on = function(ev, callback) {
    this._callbacks[ev] = this._callbacks[ev] || [];
    return this._callbacks[ev].push(callback);
  };

  Popup.prototype.off = function(ev) {
    if (ev) {
      return this._callbacks[ev] = [];
    }
    return this._callbacks = {};
  };

  Popup.prototype.trigger = function(ev, data) {
  ...
  Popup.prototype.hide = function() {
    return this._sendEvent('manage', {
      "do": 'hide'
    }).then(this._saveInfo).then(this._hide);
  };
  ...
  Popup.prototype.show = function() {
    ...
    return this.position().then(function() {
      return _this._sendEvent('manage', {
        "do": 'show'
      }).then(_this._saveInfo).then(function() {
  ...
  Popup.prototype._saveInfo = function(response) {
  ...
  Popup.prototype._prepare = function(window) {
    ...
    sheets = Array.prototype.map.call(sheets, function(el) {
      return el.href;
    });
    if (!(this.css || sheets.length)) {
      return this;
    }
    return loadSheets(this.head, this.css, sheets).then(function() {
...
},{"./../../../../bower/aye/ayepromise.js":1,"config":"ZaiTg0","json3":5,"utils/event":"OZFmFt","utils/guid":"EadS8b"}],"api/index":[function(require,module,exports){
...
api = function(url, method, data) {
  var advance, deffered, id, message, promise;
  id = guid();
  promise = (defs[id] = deffered = aye.defer()).promise;
  message = prepare_message.apply(this, deffered.margs = [url, method, data, id]);
  advance = function() {
    return defs[id] = null;
  };
  ...
  if (a.t === 't') {
    ...
    if (link.tokens && data.error && ((_ref = data.error) === 'invalid_grant' || _ref === 'access_denied')) {
      ...
      return requestToken().then(ok, (function() {
        tokenDef = null;
        return deffered.reject(new Error('Unable to login again!'));
      }));
    } else {
      return deffered.reject(new Error(data.error_description));
    }
  }
};
...
},{"../auth/index":"9joUsL","../config":"ZaiTg0","../utils/guid":"EadS8b","../utils/queue":"GkoH8v","./../../../bower/aye/ayepromise.js":1,"./../../../bower/pubsub.js/src/pubsub.js":2,"./request":"kgtKz9","json3":5}],"kgtKz9":[function(require,module,exports){
var config, json, request, uxhr;
...
request = function(params) {
  ...
  complete = function(response, status) {
    ...
    if (config.crossdomain) {
      ...
      return window.widgeticReceiver({
...
},{"../config":"ZaiTg0","./../../../bower/uxhr/uxhr.js":4,"json3":5}],"api/request":[function(require,module,exports){
...
},{}],"auth/index":[function(require,module,exports){
...
},{"../api":"pK1+ma","../config":"ZaiTg0","./../../../bower/aye/ayepromise.js":1,"./iframe":"67urtb","./popup":"JLc0KN"}],"JLc0KN":[function(require,module,exports){
...
var config, domain, domainProtocol, host, o, parse, parsedDomain, protocol, wl, _ref, _ref1, _ref2, _ref3;
...
domainProtocol = ((_ref1 = window.widgeticOptions) != null ? _ref1.domainProtocol : void 0) || 'https';

o = "?lo=" + (encodeURIComponent(wl.origin));

protocol = ((_ref2 = window.widgeticOptions) != null ? _ref2.secure : void 0) === false ? 'http' : 'https';
...
config = {
  ...
  crossdomain: ((_ref3 = window.widgeticOptions) != null ? _ref3.crossdomain : void 0) || true
};

module.exports = config;


},{"./detect/parse":"mM0D7K"}],"KcPy++":[function(require,module,exports){
module.exports = {
...
},{}],"constants/events":[function(require,module,exports){
...
},{"../config":"ZaiTg0","./parse":"mM0D7K","json3":5}],"detect/parse":[function(require,module,exports){
module.exports=require('mM0D7K');
},{}],"mM0D7K":[function(require,module,exports){
var parse;
...
},{}],"dom/root":[function(require,module,exports){
module.exports=require('+aqgVi');
},{}],"+aqgVi":[function(require,module,exports){
var Root, api, aye, config, css, event, iframe, proxyCreated, steps;
  ...
  if (steps.init) {
    if (config.crossdomain) {
      ...
      fail = function() {
        ...
        return proxyCreated.reject(new Error('Could not initialize cross-domain messaging iframe'));
      };
      timeout = setTimeout(fail, 10000);
      Root._done = function() {
...
},{"../api/index":"pK1+ma","../auth/iframe":"67urtb","../config":"ZaiTg0","../utils/event":"OZFmFt","./../../../bower/aye/ayepromise.js":1}],"index":[function(require,module,exports){
module.exports=require('tzAnED');
},{}],"tzAnED":[function(require,module,exports){
var Composition, Editor, JSON, Root, UI, Widgetic, api, auth, aye, callReceiver, config, detect, event, hasProxy, initProxy, msgReceiver, originRegex, parseData, proxyInitialized, proxyReceiver, receivers, resizeFrame, whenReady, win, winP;
...
resizeFrame = function(data) {
  var ifr;
  ifr = document.querySelector("[name=\"" + data.d.source + "\"]");
  ifr || (ifr = document.querySelector("[name=\"" + data.id + "\"]"));
  return ifr.parentElement.style.height = data.d.height + 'px';
};
...
parseData = function(data) {
  ...
  if (typeof data !== 'string') {
    ...
    try {
      ...
      def.reject(new Error("error parsing JSON: " + data));
    }
  }
  return def.promise;
};

proxyReceiver = function(e, data) {
  var comp, sourceName, _ref;
  if (e.source === winP) {
  ...
  data.d.origSource = data.d.source || sourceName;
  data.d.source = win.name;
  if ((_ref = data.d.anchor) != null) {
    _ref.parent = win.name;
  }
  return winP.postMessage(JSON.stringify(data), '*');
};

callReceiver = function(e, data) {
  var isProxy, receiver, sourceName, _ref;
  try {
    sourceName = e.source.name;
  } catch (_error) {}
  if (isProxy = ((_ref = data.t) === 'p' || _ref === 'su') && (e.source === winP || Composition.getComp(data.d.source || sourceName))) {
    return proxyReceiver(e, data);
  }
  if ((receiver = receivers[data.t]) == null) {
    throw new Error("No receiver for " + data.t + "!");
  }
  return receiver(data, e);
};

msgReceiver = function(e) {
  if (!(originRegex.test(e.origin) || e.source === winP)) {
    return;
  }
  return parseData(e.data).then(callReceiver.bind(null, e)).fail(function(error) {
    if (!error) {
      return;
    }
    ...
    return console.warn("Widgetic SDK: ", error);
  });
};
...
initProxy = function() {
  ...
  if (!hasProxy) {
    create = function() {
      (_this.root = new Root()).createProxy().then(function() {
        return proxyInitialized.resolve();
      }).fail(function(e) {
        return proxyInitialized.reject(e);
      });
...
Widgetic.prototype.api = function() {
  var args;
  args = arguments;
  return initProxy().then(function() {
...
},{}],"utils/queue":[function(require,module,exports){
...
},{"./../../../bower/queue-async/queue.js":3}],"utils/ready":[function(require,module,exports){
