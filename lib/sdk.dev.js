...
(function() {
  ...
  function queue(parallelism) {
    ...
    function notify() {
      if (error != null) awaitz(error);
      else if (all) awaitz(error, tasks);
      else awaitz.apply(null, [error].concat(tasks));
    }
...
Composition = function(holder, opt1, opt2) {
  ...
  if (options.iframeStyle) {
    this._iframe.setAttribute('style', options.iframeStyle);
  }
  this._iframe.style.visibility = 'hidden';
  this._iframe.onload = function() {
    return _this._iframe.style.visibility = 'visible';
  };
...
Composition.updateSize = function(data) {
  return comps[data.id].updateSize(data.d.width, data.d.height);
};
...
},{"./composition":"aP+Ks/","./editor":"sk8aR+","./parse":"RadvXf","./plugin":"vqfNgY","./popup":"7FyBBT"}],"UI/parse":[function(require,module,exports){
module.exports=require('RadvXf');
},{}],"RadvXf":[function(require,module,exports){
var defaultResizeStyle, embed, getHolder, parse, parseShortcodes, replaceParentWithChild, resizeHolderTemplate, stylesFactory, whenReady;
...
parseShortcodes = function() {
  var children, el, extraWhitespace, newEl, newValue, parentEl, shortcode, toAnchorTag, value, _i, _len, _ref, _ref1, _results;
  shortcode = /\[widgetic\-([-\w]+) *((?: [-\w]+(?:=.*?)?)+)\]/ig;
  extraWhitespace = /\s+/g;
  toAnchorTag = function(_, widget, attributes) {
    var htmlAttributes;
    htmlAttributes = attributes.trim().replace(extraWhitespace, ' ').split(' ').map(function(pair) {
      return 'data-' + pair;
    }).join(' ');
    return '<a href="https://widgetic.com/widget/' + widget + '/" class="widgetic-composition" ' + htmlAttributes + '></a>';
  };
  _ref = document.querySelectorAll("body, body *");
  _results = [];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    parentEl = _ref[_i];
    if ((_ref1 = parentEl.nodeName) === 'HEAD' || _ref1 === 'META' || _ref1 === 'LINK' || _ref1 === 'SCRIPT' || _ref1 === 'TITLE') {
      continue;
    }
    ...
    _results.push((function() {
      ...
      for (_j = 0, _len1 = children.length; _j < _len1; _j++) {
        ...
        if (value !== newValue) {
          ...
          _results1.push(parentEl.replaceChild(newEl, el));
        } else {
...
embed = function(el) {
  ...
  return composition = new Widgetic.UI.composition(holder, options.composition, options);
};
...
loadSheets = function(el, sheets, optional, callback) {
  var allSheets, loadedSheets, onLoad, sheet, total, waiting, _i, _len,
    _this = this;
  if (sheets == null) {
    sheets = [];
  }
...
\
  ...
  Popup.prototype._prepare = function(window) {
    ...
    this.loader.innerHTML = '<div class="sk-three-bounce">\
				<div class="sk-child sk-bounce1"></div>\
				<div class="sk-child sk-bounce2"></div>\
				<div class="sk-child sk-bounce3"></div>\
			</div>';
    this.document.body.appendChild(this.loader);
    this.content = document.createElement('div');
    this.document.body.appendChild(this.content);
...
var Composition, Editor, JSON, Root, UI, Widgetic, api, auth, aye, config, detect, event, hasProxy, initProxy, originRegex, proxyInitialized, receiver, receivers, whenReady, win;
...
window.widgeticReceiver = receiver = function(e) {
  var d, error, _name;
...
Widgetic.prototype.VERSION = '0.6.5';
