...
Composition = function(holder, data, brand_pos) {
  ...
  if (token = data.token || api.accessToken()) {
...
var api, app, auth, aye, config, iframe, lastScope, link, popup, url, _get;
...
auth.register = function(scope) {
  ...
  return oa(url(scope, 'signup'), deffered);
};
...
var Blogvio, Composition, Editor, JSON, Root, UI, api, auth, config, detect, event, hasProxy, initProxy, originRegex, receiver, receivers, whenReady, win;
...
originRegex = RegExp("" + (config.lo.replace(/(http|https)\:/, '')) + "|" + (config.domain.replace(/(http|https)\:/, '')));
...
Blogvio.prototype.auth.register = function() {
